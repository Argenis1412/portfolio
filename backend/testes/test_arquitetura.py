...
def get_imports_from_file(file_path):
    ...
    imports = re.findall(r"^(?:import|from)\s+(app\.[a-zA-Z0-9_\.]+)", content, re.MULTILINE)
    
    # Procura por "import fastapi..." e "from fastapi..."
    third_party = re.findall(r"^(?:import|from)\s+(fastapi|pydantic|sqlalchemy|sqlmodel)", content, re.MULTILINE)
    ...
    for py_file in entidades_dir.glob("*.py"):
        ...
        for imp in app_imports:
            ...
            if not imp.startswith("app.entidades") and not imp.startswith("app.core"):
                raise AssertionError(f"Violação de Arquitetura: {py_file.name} importa {imp}")
        
        # Verificar dependências externas proibidas
        proibidos = {"fastapi", "pydantic", "sqlalchemy", "sqlmodel"}
        for imp in ext_imports:
            if imp in proibidos:
                raise AssertionError(f"Violação de PUREZA: {py_file.name} importa framework externo: {imp}")
    ...
    for py_file in casos_uso_dir.glob("*.py"):
        ...
        for imp in app_imports:
            if imp.startswith("app.controladores"):
                raise AssertionError(f"Violação de Arquitetura: Caso de Uso {py_file.name} importa Controlador {imp}")
        
        # Verificar dependências web proibidas
        for imp in ext_imports:
            if imp == "fastapi":
                raise AssertionError(f"Violação de Arquitetura: Caso de Uso {py_file.name} importa FastAPI")
