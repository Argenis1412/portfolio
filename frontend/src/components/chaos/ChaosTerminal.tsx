        ...
        <AnimatePresence mode="popLayout">
          {entries.map((entry) => {
            ...
            const formattedMessage = parts.map((part, i) => {
              if (part === 'TIMEOUT' || part === 'CIRCUIT_BREAKER=OPEN') return (
                <m.span
                  key={i}
                  initial={prefersReducedMotion ? false : { scale: 0.85 }}
                  animate={{ scale: 1 }}
                  transition={{ duration: 0.2 }}
                  className="text-status-error font-bold bg-status-error-soft px-1 rounded inline-block"
                >
                  {part}
                </m.span>
              );
              if (part === 'DEGRADED') return (
                <m.span
                  key={i}
                  initial={prefersReducedMotion ? false : { scale: 0.85 }}
                  animate={{ scale: 1 }}
                  transition={{ duration: 0.2 }}
                  className="text-status-error font-bold inline-block"
                >
                  {part}
                </m.span>
              );
              if (part === 'RETRY' || part === 'FALLBACK') return (
                <m.span
                  key={i}
                  initial={prefersReducedMotion ? false : { scale: 0.85 }}
                  animate={{ scale: 1 }}
                  transition={{ duration: 0.2 }}
                  className="text-status-warn font-bold bg-status-warn-soft px-1 rounded inline-block"
                >
                  {part}
                </m.span>
              );
              if (part === 'RECOVERED' || part === 'status=APPLIED') return (
                <m.span
                  key={i}
                  initial={prefersReducedMotion ? false : { scale: 0.85 }}
                  animate={{ scale: 1 }}
                  transition={{ duration: 0.2 }}
                  className="text-status-ok font-bold inline-block"
                >
                  {part}
                </m.span>
              );
